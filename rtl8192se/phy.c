		...
		enum radio_path rfpath, u32 offset)
{
	...
	RT_TRACE(COMP_RF, DBG_TRACE, ("RFR-%d Addr[0x%x]=0x%x\n", rfpath,
			pphyreg->rflssi_readback, retvalue));
			...
			u8 operation)
{
	...
	if (!is_hal_stop(rtlhal)) {
		...
		case SCAN_OPT_RESTORE:
			...
			break;
		default:
			RT_TRACE(COMP_ERR, DBG_EMERG, ("Unknown operation.\n"));
			break;
		}
	}
}
		...
		enum nl80211_channel_type ch_type)
{
	...
	RT_TRACE(COMP_SCAN, DBG_TRACE, ("Switch to %s bandwidth\n",
	     ...
	     u8 channel, u8 *stage, u8 *step, u32 *delay)
{
	...
	_rtl92s_phy_set_sw_chnl_cmdarray(rfdependcmd, rfdependcmdcnt++,
			MAX_RFDEPENDCMD_CNT, CMDID_RF_WRITEREG, RF_CHNLBW, channel, 10);
		...
		case CMDID_RF_WRITEREG:
			for (rfpath = 0; rfpath < num_total_rfpath; rfpath++) {
				...
				rtl_set_rfreg(hw, (enum radio_path)rfpath, currentcmd->para1,
						RFREG_OFFSET_MASK, rtlphy->rfreg_chnlval[rfpath]);
			}
			break;
		default:
			RT_TRACE(COMP_ERR, DBG_EMERG,
...
u8 rtl92s_phy_sw_chnl(struct ieee80211_hw *hw)
{
	...
	do {
		...
		ret = _rtl92s_phy_sw_chnl_step_by_step(hw, rtlphy->current_channel,
					    ...
					    enum rf_pwrstate rfpwr_state)
{
	...
	case ERFON:{
			if ((ppsc->rfpwr_state == ERFOFF) &&
				...
				RT_TRACE(COMP_POWER, DBG_DMESG, ("awake, sleeped:%d ms "
	...
	case ERFSLEEP:
			...
			for (queue_id = 0, i = 0;
				...
				if (skb_queue_len(&ring->queue) == 0 ||
					...
					RT_TRACE(COMP_ERR, DBG_WARNING, ("eRf Off/Sleep: "
						  "%d times TcbBusyQueue[%d] =%d before doze!\n",
			...
			RT_TRACE(COMP_POWER, DBG_DMESG, ("Set ERFSLEEP awaked:%d ms\n",
...
static void _rtl92s_phy_init_register_definition(struct ieee80211_hw *hw)
{
	...
	rtlphy->phyreg_def[RF90_PATH_C].rf3wire_offset = RFPGA0_XC_LSSIPARAMETER;
	rtlphy->phyreg_def[RF90_PATH_D].rf3wire_offset = RFPGA0_XD_LSSIPARAMETER;
	...
	/* RF switch Control */
	rtlphy->phyreg_def[RF90_PATH_A].rfswitch_control = RFPGA0_XAB_SWITCHCONTROL;
	rtlphy->phyreg_def[RF90_PATH_B].rfswitch_control = RFPGA0_XAB_SWITCHCONTROL;
	rtlphy->phyreg_def[RF90_PATH_C].rfswitch_control = RFPGA0_XCD_SWITCHCONTROL;
	rtlphy->phyreg_def[RF90_PATH_D].rfswitch_control = RFPGA0_XCD_SWITCHCONTROL;
	...
	/* RX AFE control 1  */
	rtlphy->phyreg_def[RF90_PATH_A].rfrxiq_imbalance = ROFDM0_XARXIQIMBALANCE;
	rtlphy->phyreg_def[RF90_PATH_B].rfrxiq_imbalance = ROFDM0_XBRXIQIMBALANCE;
	rtlphy->phyreg_def[RF90_PATH_C].rfrxiq_imbalance = ROFDM0_XCRXIQIMBALANCE;
	rtlphy->phyreg_def[RF90_PATH_D].rfrxiq_imbalance = ROFDM0_XDRXIQIMBALANCE;
	...
	/* Tx AFE control 1  */
	rtlphy->phyreg_def[RF90_PATH_A].rftxiq_imbalance = ROFDM0_XATXIQIMBALANCE;
	rtlphy->phyreg_def[RF90_PATH_B].rftxiq_imbalance = ROFDM0_XBTXIQIMBALANCE;
	rtlphy->phyreg_def[RF90_PATH_C].rftxiq_imbalance = ROFDM0_XCTXIQIMBALANCE;
	rtlphy->phyreg_def[RF90_PATH_D].rftxiq_imbalance = ROFDM0_XDTXIQIMBALANCE;
	...
	rtlphy->phyreg_def[RF90_PATH_A].rflssi_readback = RFPGA0_XA_LSSIREADBACK;
	rtlphy->phyreg_def[RF90_PATH_B].rflssi_readback = RFPGA0_XB_LSSIREADBACK;
	rtlphy->phyreg_def[RF90_PATH_C].rflssi_readback = RFPGA0_XC_LSSIREADBACK;
	rtlphy->phyreg_def[RF90_PATH_D].rflssi_readback = RFPGA0_XD_LSSIREADBACK;

	/* Tranceiver LSSI Readback PI mode  */
	rtlphy->phyreg_def[RF90_PATH_A].rflssi_readbackpi = TRANSCEIVERA_HSPI_READBACK;
	rtlphy->phyreg_def[RF90_PATH_B].rflssi_readbackpi = TRANSCEIVERB_HSPI_READBACK;
}
...
static bool _rtl92s_phy_config_bb_with_pg(struct ieee80211_hw *hw, u8 configtype)
{
	...
	if (configtype == BASEBAND_CONFIG_PHY_REG) {
		for (i = 0; i < phy_pg_len; i = i + 3) {
			...
			_rtl92s_store_pwrindex_diffrate_offset(hw, phy_table_pg[i],
					phy_table_pg[i + 1], phy_table_pg[i + 2]);
...
static bool _rtl92s_phy_bb_config_parafile(struct ieee80211_hw *hw)
{
	...
	if (rtstatus != true){
		RT_TRACE(COMP_INIT, DBG_EMERG, ("Write BB Reg Fail!!"));
	...
	if (rtstatus != true){
		printk( "_rtl92s_phy_bb_config_parafile():AGC Table Fail\n");
		goto phy_BB8190_Config_ParaFile_Fail;
	}
	...
	case RF90_PATH_A:
		for (i = 0; i < radio_a_tblen; i = i + 2) {
			if (radio_a_table[i] == 0xfe)
				...
				rtl92s_phy_set_rf_reg(hw, rfpath, radio_a_table[i],
	...
	case RF90_PATH_B:
		for (i = 0; i < radio_b_tblen; i = i + 2) {
			if (radio_b_table[i] == 0xfe)
				...
				rtl92s_phy_set_rf_reg(hw, rfpath, radio_b_table[i],
...
bool rtl92s_phy_bb_config(struct ieee80211_hw *hw)
{
	...
	pathmap = (u8)(rtl92s_phy_query_bb_reg(hw, RFPGA0_TXINFO, 0xf) |
			rtl92s_phy_query_bb_reg(hw, ROFDM0_TRXPATHENABLE, 0xf));
	...
	if ((rtlphy->rf_type == RF_1T1R && rf_num != 1) ||
		...
		RT_TRACE(COMP_INIT, DBG_EMERG, ("RF_Type(%x) does not match"
...
bool rtl92s_phy_rf_config(struct ieee80211_hw *hw)
{
	...
	/* Config BB and RF */
	rtstatus = rtl92s_phy_rf6052_config(hw);
...
void rtl92s_phy_get_hw_reg_originalvalue(struct ieee80211_hw *hw)
{
	...
	RT_TRACE(COMP_INIT, DBG_LOUD, ("Default initial gain "
...
void rtl92s_phy_chk_fwcmd_iodone(struct ieee80211_hw *hw)
{
	...
	if (pollingcnt == 0)
		RT_TRACE(COMP_ERR, DBG_EMERG, ("Set FW Cmd fail!!\n"));
}
...
static void _rtl92s_phy_set_fwcmd_io(struct ieee80211_hw *hw)
{
	...
	case FW_CMD_RA_RESET:
		RT_TRACE(COMP_CMD, DBG_DMESG, ("FW_CMD_RA_RESET\n"));
		rtl_write_dword(rtlpriv, WFM5, FW_RA_RESET);
		rtl92s_phy_chk_fwcmd_iodone(hw);
		break;
	case FW_CMD_RA_ACTIVE:
		RT_TRACE(COMP_CMD, DBG_DMESG, ("FW_CMD_RA_ACTIVE\n"));
		rtl_write_dword(rtlpriv, WFM5, FW_RA_ACTIVE);
		rtl92s_phy_chk_fwcmd_iodone(hw);
		break;
	case FW_CMD_RA_REFRESH_N:
		RT_TRACE(COMP_CMD, DBG_DMESG, ("FW_CMD_RA_REFRESH_N\n"));
		input = FW_RA_REFRESH;
		...
		break;
	case FW_CMD_RA_REFRESH_BG:
		RT_TRACE(COMP_CMD, DBG_DMESG, ("FW_CMD_RA_REFRESH_BG\n"));
	...
	case FW_CMD_RA_REFRESH_N_COMB:
		RT_TRACE(COMP_CMD, DBG_DMESG, ("FW_CMD_RA_REFRESH_N_COMB\n"));
		input = FW_RA_IOT_N_COMB;
		rtl_write_dword(rtlpriv, WFM5, input);
		rtl92s_phy_chk_fwcmd_iodone(hw);
		break;
	case FW_CMD_RA_REFRESH_BG_COMB:
		RT_TRACE(COMP_CMD, DBG_DMESG, ("FW_CMD_RA_REFRESH_BG_COMB\n"));
		input = FW_RA_IOT_BG_COMB;
		rtl_write_dword(rtlpriv, WFM5, input);
		rtl92s_phy_chk_fwcmd_iodone(hw);
		break;
	case FW_CMD_IQK_ENABLE:
		RT_TRACE(COMP_CMD, DBG_DMESG, ("FW_CMD_IQK_ENABLE\n"));
	...
	case FW_CMD_LPS_ENTER:
		RT_TRACE(COMP_CMD, DBG_DMESG, ("FW_CMD_LPS_ENTER\n"));
	...
	case FW_CMD_LPS_LEAVE:
		RT_TRACE(COMP_CMD, DBG_DMESG, ("FW_CMD_LPS_LEAVE\n"));
		rtl_write_dword(rtlpriv, WFM5, FW_LPS_LEAVE );
		rtl92s_phy_chk_fwcmd_iodone(hw);
		break;
	case FW_CMD_ADD_A2_ENTRY:
		RT_TRACE(COMP_CMD, DBG_DMESG, ("FW_CMD_ADD_A2_ENTRY\n"));
		rtl_write_dword(rtlpriv, WFM5, FW_ADD_A2_ENTRY);
		rtl92s_phy_chk_fwcmd_iodone(hw);
		break;
	case FW_CMD_CTRL_DM_BY_DRIVER:
		RT_TRACE(COMP_CMD, DBG_LOUD, ("FW_CMD_CTRL_DM_BY_DRIVER\n"));
...
bool rtl92s_phy_set_fw_cmd(struct ieee80211_hw *hw, enum fwcmd_iotype fw_cmdio)
{
	...
	do {
		...
		switch (fw_cmdio) {
		case FW_CMD_RA_INIT:
			RT_TRACE(COMP_CMD, DBG_LOUD, ("RA init!!\n"));
		...
		case FW_CMD_DIG_DISABLE:
			RT_TRACE(COMP_CMD, DBG_LOUD, ("Set DIG disable!!\n"));
		...
		case FW_CMD_DIG_HALT:
			RT_TRACE(COMP_CMD, DBG_LOUD, ("Set DIG halt!!\n"));
			fw_cmdmap &= ~(FW_DIG_ENABLE_CTL | FW_SS_CTL);
			FW_CMD_IO_SET(rtlpriv, fw_cmdmap);
			break;
		case FW_CMD_TXPWR_TRACK_THERMAL: {
			...
			fw_param |= ((thermalval << 24) | (rtlefuse->thermalmeter[0] <<
					16));

			RT_TRACE(COMP_CMD, DBG_LOUD, ("Set TxPwr tracking!! "
		...
		case FW_CMD_RA_REFRESH_N_COMB:
			...
			RT_TRACE(COMP_CMD, DBG_LOUD, ("[FW CMD] [New Version] "
		...
		case FW_CMD_RESUME_DM_BY_SCAN:
			...
			if ((digtable.dig_ext_port_stage == DIG_EXT_PORT_STAGE_0) ||
				(digtable.dig_ext_port_stage == DIG_EXT_PORT_STAGE_1))
				fw_cmdmap &= ~FW_DIG_ENABLE_CTL;
		...
		case FW_CMD_HIGH_PWR_ENABLE:
			if (!(rtlpriv->dm.dm_flag & HAL_DM_HIPWR_DISABLE) &&
				(rtlpriv->dm.bdynamic_txpower_enable != true)) {
				fw_cmdmap |= (FW_HIGH_PWR_ENABLE_CTL | FW_SS_CTL);
				FW_CMD_IO_SET(rtlpriv, fw_cmdmap);
				bPostProcessing = true;
			}
			break;
		...
		case FW_CMD_PAPE_CONTROL:
			RT_TRACE(COMP_CMD, DBG_LOUD, ("[FW CMD] Set PAPE Control \n"));
