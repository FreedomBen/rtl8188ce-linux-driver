	...
	u8 rpwm_val)
{
	...
	if (FW_PS_STATE(rtlhal->fw_ps_state) != FW_PS_STATE_RF_OFF_LOW_PWR_88E) {
		spin_lock_bh(&rtlpriv->locks.fw_ps_lock);
		if (!rtlhal->bfw_clk_change_in_progress) {
...
static bool _rtl88ee_llt_table_init(struct ieee80211_hw *hw)
{
	...
	rtl_write_dword(rtlpriv, REG_TRXFF_BNDY, (0x25FF0000 | txpktbuf_bndy));/*0x2600   MaxRxBuff=10k-max(TxReportSize(64*8), WOLPattern(16*24)). YJ,111031*/
...
static bool _rtl88ee_init_mac(struct ieee80211_hw *hw)
{
	...
	if(rtlhal->b_earlymode_enable){/*Early mode enable*/
...
void rtl8188ee_allow_error_packet(struct ieee80211_hw *hw,bool b_allow_err_pkt)
{
	struct rtl_priv *rtlpriv = rtl_priv(hw);
	struct rtl_pci *rtlpci = rtl_pcidev(rtl_pcipriv(hw));
...
int rtl88ee_hw_init(struct ieee80211_hw *hw)
{
	...
	RT_TRACE(COMP_INIT,DBG_LOUD,(" Rtl8188EE hw init\n"));
	...
	if (ppsc->rfpwr_state == ERFON) {
		...
		RT_TRACE(COMP_INIT,DBG_LOUD,("rx idle ant %s\n",(rtlpriv->dm.fat_table.rx_idle_ant == MAIN_ANT) ?
			("MAIN_ANT") : ("AUX_ANT")));
	...
	if (!(tmp_u1b & BIT(4))) {
		...
		RT_TRACE(COMP_INIT, DBG_LOUD, ("under 1.5V\n"));
	}
	rtl_write_byte(rtlpriv, REG_NAV_CTRL+2,  ((30000+127)/128));
	rtl88e_dm_init(hw);
	rtlpriv->rtlhal.being_init_adapter = false;
	RT_TRACE(COMP_INIT,DBG_LOUD,("end of Rtl8188EE hw init %x\n",err));
	return 0;
}
...
static enum version_8188e _rtl88ee_read_chip_version(struct ieee80211_hw *hw)
{
	...
	if (value32 & TRP_VAUX_EN) {
		...
		version = version | ((value32 & VENDOR_ID) ? CHIP_VENDOR_UMC : 0);
	}
...
void rtl88ee_card_disable(struct ieee80211_hw *hw)
{
	...
	RT_TRACE(COMP_INIT,DBG_LOUD,("RTL8188ee card disable\n"));
	...
	u8 *hwinfo)
{
	...
	RT_TRACE(COMP_INIT, DBG_LOUD, ("hal_ReadPowerValueFromPROM88E(): PROMContent[0x%x]=0x%x\n", (eeAddr+1), hwinfo[eeAddr+1]));
						 ...
						 u8 *hwinfo)
{
	...
	_rtl8188e_read_power_value_fromprom(hw, &pwrinfo24g, &pwrinfo5g, autoload_fail, hwinfo);
	...
	for (rf_path = 0; rf_path < 2; rf_path++) {
		for (i = 0; i < 14; i++) {
			...
			if (i == 14)
				rtlefuse->txpwrlevel_ht40_1s[rf_path][i] = pwrinfo24g.index_bw40_base[rf_path][4];
			else
				rtlefuse->txpwrlevel_ht40_1s[rf_path][i] = pwrinfo24g.index_bw40_base[rf_path][index];	
			rtlefuse->txpwr_ht20diff[rf_path][i] = pwrinfo24g.bw20_diff[rf_path][0];
			rtlefuse->txpwr_legacyhtdiff[rf_path][i] = pwrinfo24g.ofdm_diff[rf_path][0];
		}
	...
	if (!autoload_fail){
		rtlefuse->eeprom_regulatory = hwinfo[EEPROM_RF_BOARD_OPTION_88E] & 0x07;/*bit0~2*/
...
static void _rtl88ee_read_adapter_info(struct ieee80211_hw *hw)
{
	...
	/*board type*/
	rtlefuse->board_type = (((*(u8 *) & hwinfo[EEPROM_RF_BOARD_OPTION_88E]) & 0xE0 ) >> 5);
	rtlhal->boad_type = rtlefuse->board_type;
	/*Wake on wlan*/
	rtlefuse->wowlan_enable = ((hwinfo[EEPROM_RF_FEATURE_OPTION_88E] & 0x40) >> 6);
	/*parse xtal*/
	rtlefuse->crystalcap = hwinfo[EEPROM_XTAL_88E];
	if (hwinfo[EEPROM_XTAL_88E])
		rtlefuse->crystalcap = 0x20;
	/*antenna diversity*/
	rtlefuse->antenna_div_cfg = (hwinfo[EEPROM_RF_BOARD_OPTION_88E] & 0x18) >>3;
	if (hwinfo[EEPROM_RF_BOARD_OPTION_88E] == 0xFF)
		rtlefuse->antenna_div_cfg = 0;
	if (rtlpcipriv->bt_coexist.eeprom_bt_coexist !=0 && rtlpcipriv->bt_coexist.eeprom_bt_ant_num == ANT_X1)
		rtlefuse->antenna_div_cfg = 0;
	...
	if (rtlhal->oem_id == RT_CID_DEFAULT) {
		...
		case EEPROM_CID_DEFAULT:
			if (rtlefuse->eeprom_did == 0x8179) {
				...
				else if (rtlefuse->eeprom_svid == 0x103c && rtlefuse->eeprom_smid == 0x197d)
					rtlhal->oem_id = RT_CID_819x_HP;
				else
					      ...
					      bool auto_load_fail, u8 *hwinfo)
{
	...
	if (!auto_load_fail) {
		...
		rtlpcipriv->bt_coexist.eeprom_bt_ant_isolation = ((value & 0x10) >> 4);
		rtlpcipriv->bt_coexist.eeprom_bt_radio_shared = ((value & 0x20) >> 5);
	} else {
		rtlpcipriv->bt_coexist.eeprom_bt_coexist = 0;
