...
if inGitRepo; then
    ...
    base="$(basename $(pwd))"
    if ( $(uname -r | grep "4.19" > /dev/null) && ! $(echo "$base" | grep "generic-4.19.x" > /dev/null) ) ||                                                          \
       ( $(uname -r | grep "4.18" > /dev/null) && ! $(echo "$base" | grep "generic-4.18.x" > /dev/null) ) && ! $(echo "$base" | grep "ubuntu-18.10"   > /dev/null) || \
