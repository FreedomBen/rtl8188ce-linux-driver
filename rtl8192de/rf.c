...
static void _rtl92d_phy_get_power_base(struct ieee80211_hw *hw,
		u8 *ppowerlevel, u8 channel, u32 *ofdmbase, u32 *mcsbase)
{
	struct rtl_priv *rtlpriv = rtl_priv(hw);
	struct rtl_phy *rtlphy = &(rtlpriv->phy);
	struct rtl_efuse *rtlefuse = rtl_efuse(rtl_priv(hw));
...
static void _rtl92d_get_txpower_writeval_by_regulatory(struct ieee80211_hw *hw,
			u8 channel, u8 index, u32 *powerbase0,
	...
	for (rf = 0; rf < 2; rf++) {
		...
		case 0:
			...
			writeval = rtlphy->mcs_txpwrlevel_origoffset[chnlgroup][index +
					...
					powerbase1[rf]);
			RTPRINT(rtlpriv, FPHY, PHY_TXPWR, ("RTK better performance, "
				 "writeval(%c) = 0x%x\n", ((rf == 0) ? 'A' : 'B'), writeval));
			break;
		case 1:
			...
			if (rtlphy->pwrgroup_cnt >= MAX_PG_GROUP) {
				...
				writeval = rtlphy->mcs_txpwrlevel_origoffset[chnlgroup][index +
						(rf ? 8 : 0)] + ((index < 2) ? powerbase0[rf] :
						powerbase1[rf]);
				RTPRINT(rtlpriv, FPHY, PHY_TXPWR, ("Realtek regulatory, "
		...
		case 3:
			...
			if (rtlphy->current_chan_bw == HT_CHANNEL_WIDTH_20_40) {
				RTPRINT(rtlpriv, FPHY, PHY_TXPWR, ("customer's limit, 40MHz "
					 "rf(%c) = 0x%x\n", ((rf == 0) ? 'A' : 'B'),
					 rtlefuse->pwrgroup_ht40[rf][channel - 1]));
			} else {
				RTPRINT(rtlpriv, FPHY, PHY_TXPWR, ("customer's limit, 20MHz "
					 "rf(%c) = 0x%x\n", ((rf == 0) ? 'A' : 'B'),
			...
			for (i = 0; i < 4; i++) {
				pwr_diff_limit[i] = (u8) ((rtlphy->mcs_txpwrlevel_origoffset[
					...
					(i * 8));
				if (rtlphy->current_chan_bw == HT_CHANNEL_WIDTH_20_40) {
					if (pwr_diff_limit[i] > rtlefuse->pwrgroup_ht40[rf][
						channel - 1])
						pwr_diff_limit[i] =rtlefuse->pwrgroup_ht40[rf][
								channel - 1];
				} else {
					if (pwr_diff_limit[i] > rtlefuse->pwrgroup_ht20[rf][
						channel - 1])
						pwr_diff_limit[i] = rtlefuse->pwrgroup_ht20[rf][
								channel - 1];
				}
			}
...
bool rtl92d_phy_rf6052_config(struct ieee80211_hw *hw)
{
	...
	for (rfpath = 0; rfpath < rtlphy->num_total_rfpath; rfpath++) {
		...
		case RF90_PATH_A:
		case RF90_PATH_C:
			u4_regvalue = rtl_get_bbreg(hw, pphyreg->rfintfs, BRFSI_RFENV);
			break;
		case RF90_PATH_B:
		case RF90_PATH_D:
		...
		case RF90_PATH_A:
			if (true_bpath == true)
				rtstatus = rtl92d_phy_config_rf_with_headerfile(hw,
						radiob_txt, (enum radio_path)rfpath);
			else
				rtstatus = rtl92d_phy_config_rf_with_headerfile(hw,
						radioa_txt, (enum radio_path)rfpath);
			break;
		case RF90_PATH_B:
			rtstatus =
